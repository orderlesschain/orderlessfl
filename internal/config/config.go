...
type Configuration struct {
	...
	FederatedServerPort                  string `mapstructure:"FEDERATED_SERVER_PORT"`
	...
	FederatedConnectionPoolCount         int    `mapstructure:"FEDERATED_CONNECTION_POOL_COUNT"`
...
func LoadConfig() (*Configuration, error) {
	...
	if config.TargetSystem == "orderlessfl" {
		config.IsOrderlessFL = true
	}
	if config.TargetSystem == "federatedlearning" {
		config.IsFederatedLearning = true
	}
	...
	if config.TotalNodeCount == 0 || config.TotalClientCount == 0 ||
